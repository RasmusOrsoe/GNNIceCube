...
mc_res = pd.read_csv(r'X:\speciale\results\dev_level7_mu_e_tau_oscweight_newfeats\everything_wtest\dynedge-protov2-energy-k=8-c3not3-vall-noprob\results.csv')#r'X:\speciale\results\dev_level7_mu_tau_e_retro_000\event_only_level7_all_neutrinos_retro_SRT_4mio\dynedge-E-protov2-zenith\results.csv')
...
plot_data_real = plot_data_real.sort_values(retro).sample(frac = 0.1)
...
ax[0].hist(plot_data_real[retro],bins = bins,label = 'retro', histtype = 'step')
...
ax[1].hist2d(plot_data_real[retro],plot_data_real[dynedge],bins = bins,cmap = cmap_hist)
ax[1].plot(plot_data_real[retro],plot_data_real[retro],'--', color = 'grey' )
ax[1].set_ylabel('$%s_{dynedge}$ [$log_{10}$ GeV]'%variable_label,size =  10)
ax[2].hist2d(plot_data_real[retro][neutrino_index],plot_data_real[dynedge][neutrino_index], bins = bins,cmap = cmap_hist)
ax[2].plot(plot_data_real[retro],plot_data_real[retro],'--', color = 'grey' )
ax[2].set_ylabel('$%s_{dynedge}$ [$log_{10}$ GeV]'%variable_label,size =  10)

p_x = plot_data_real[retro].mean()
p_y = plot_data_real[dynedge].max() - 4/5*plot_data_real[dynedge].max()
ax[1].text(p_x,p_y, '10% of IC86.11 OscNext', fontsize=15,  color='red')
ax[2].text(p_x,p_y, '$\\nu$-classified IC86.11 OscNext 10% sample', fontsize=15,  color='red')
...
plot_data = pd.concat( [pd.DataFrame(mc_res['event_no']),
                        ...
                        pd.DataFrame(retro_mc[retro]*const)],axis = 1)
...
ax[3].hist2d(plot_data[retro],plot_data[dynedge],bins = bins,cmap = cmap_hist)
ax[3].plot(plot_data_real[retro],plot_data_real[retro],'--', color = 'grey' )
ax[3].set_ylabel('$%s_{dynedge}$ [$log_{10}$ GeV]'%variable_label,size =  10)
ax[3].set_xlabel('$%s_{retro}$ [$log_{10}$ GeV]'%variable_label,size =  15)
...
fig.suptitle(' %s MC Distributions  ($v_{\mu, e, \\tau}$) NC + CC'%variable,size = 20)
...
axs[2, 0].set_xlabel('$%s_{dynedge}$ [$log_{10}$ GeV]'%variable_label,size =  15)
axs[2, 0].set_ylabel('$%s_{dynedge}$ [$log_{10}$ GeV]'%variable_label,size =  15)
axs[2, 0].legend()
...
#ax[3].set_xlabel('$%s_{dynedge}$ [$log_{10}$ GeV]'%variable_label,size =  15)
#ax[3].text(p_x,p_y, 'MC  ($v_{\mu, e, \\tau}$)', fontsize=20,  color='red')
...
plot_data = pd.concat( [pd.DataFrame(scalers_dyn['truth'][variable].inverse_transform(np.array(mc_res[dynedge]).reshape(-1,1))*const),
                        ...
                        pd.DataFrame(retro_mc[retro_sigma]*const)],axis = 1)
plot_data.columns = [dynedge,variable,retro,retro_sigma]
...
axs[1, 0].hist2d(plot_data[variable],plot_data[retro],bins = bins,label = 'retro',cmap = cmap_hist)
axs[1, 0].plot(plot_data[variable],plot_data[variable],'--', color = 'grey' )
axs[1, 0].set_ylabel('$%s_{retro}$ [$log_{10}$ GeV]'%variable_label,size =  15)


axs[2, 0].hist2d(plot_data[variable],plot_data[dynedge],bins = bins,label = 'retro',cmap = cmap_hist)
axs[2, 0].plot(plot_data[variable],plot_data[variable],'--', color = 'grey' )
axs[2, 0].set_xlabel('$%s_{truth}$ [$log_{10}$ GeV]'%variable_label,size =  15)
axs[2, 0].set_ylabel('$%s_{dynedge}$ [$log_{10}$ GeV]'%variable_label,size =  15)
axs[2, 0].legend()
...
axs[2, 1].set_xlabel('$%s_{truth}$ [$log_{10}$ GeV]'%variable_label,size =  15)
axs[2, 1].legend()
