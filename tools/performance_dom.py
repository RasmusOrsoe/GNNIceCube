...
results = pd.read_csv(r'X:\speciale\results\oscnext_IC8611_newfeats_000_mc_scaler\regression\dynedge-azimuth-protov2-c3not3-k=8-thesis-newlr-30e-check\results.csv')#r'X:\speciale\results\dev_level7_oscNext_IC86_003\event_only_level7_all_oscNext_IC86_003\dynedge-E-protov2-zenith\results.csv')

mc_res = pd.read_csv(r'X:\speciale\results\dev_level7_mu_e_tau_oscweight_newfeats\everything\dynedge-azimuth-protov2-c3not3-k=8-thesis-newlr-30e-check\results.csv')#r'X:\speciale\results\dev_level7_mu_tau_e_retro_000\event_only_level7_all_neutrinos_retro_SRT_4mio\dynedge-E-protov2-zenith\results.csv')
...
if variable == 'energy_log10':
    ...
    plot_data_real.columns = ['event_no',dynedge,retro,retro_sigma, 'neutrino']
...
ax[1,0].set_ylabel('dynedge',size =  20)
ax[2,0].hist2d(plot_data_real[retro][neutrino_index],plot_data_real[dynedge][neutrino_index], bins = bins)
ax[2,0].set_ylabel('dynedge', size = 20)

ax[3,0].hist2d(plot_data_real[retro][retro_pid_index],plot_data_real[dynedge][retro_pid_index], bins = bins)
ax[3,0].set_ylabel('dynedge', size = 20)
...
ax[1,0].text(p_x,p_y, 'Full IC86.11 OscNext', fontsize=15,  color='red')
...
if variable != 'energy_log10':
    ...
    ax[2,1].hist2d(plot_data_real[retro][neutrino_index],plot_data_real[dynedge][neutrino_index], bins = bins)
    ax[2,1].text(p_x,p_y, '$\\nu$-classified dynedge IC86.11 OscNext', fontsize=15,  color='red')
    ax[3,1].hist2d(plot_data_real[retro][retro_pid_index],plot_data_real[dynedge][retro_pid_index], bins = bins)
    ...
    ax[1,1].text(p_x,p_y, 'Full IC86.11 OscNext', fontsize=15,  color='red')
    ...
    ax_sigma[1,1].hist2d(plot_data_real[retro_sigma][not_nan],plot_data_real[sigma][not_nan], bins = np.arange(0,50,1))
...
if variable == 'azimuth':
    ...
    plot_data = pd.concat( [pd.DataFrame(mc_res['event_no']),
                        ...
                        pd.DataFrame(retro_mc['interaction_type']),
                        pd.DataFrame(strings['n_doms'])],axis = 1)
    plot_data.columns = ['event_no',dynedge,variable,retro,sigma,'energy_log10','pid','interaction_type','n_strings']
...
if variable == 'zenith':
    plot_data = pd.concat( [pd.DataFrame(mc_res['event_no']).reset_index(drop = True),
                            pd.DataFrame(scalers_dyn['truth'][variable].inverse_transform(np.array(mc_res[dynedge]).reshape(-1,1))*const).reset_index(drop = True),
                            pd.DataFrame(scalers_dyn['truth'][variable].inverse_transform(np.array(mc_res[variable]).reshape(-1,1))*const).reset_index(drop = True),
                            pd.DataFrame(retro_mc[retro]*const).reset_index(drop = True),
                            pd.DataFrame(1/np.sqrt(abs(mc_res[sigma].reset_index(drop = True)))*const),
                            pd.DataFrame(scalers_dyn['truth']['energy_log10'].inverse_transform(np.array(retro_mc['energy_log10'].reset_index(drop = True)).reshape(-1,1))),
                            pd.DataFrame(retro_mc['pid'].reset_index(drop = True)),
                            pd.DataFrame(np.array(retro_mc['osc_weight'].reset_index(drop = True))),
                            pd.DataFrame(retro_mc['interaction_type'].reset_index(drop = True)),
                            pd.DataFrame(strings['n_doms'].reset_index(drop = True))],axis = 1)
    plot_data.columns = ['event_no',dynedge,variable,retro,
                         sigma,'energy_log10','pid',
                         'osc_weight','interaction_type','n_strings']
...
if variable != 'energy_log10':
    ax[4,0].hist2d(plot_data[retro],plot_data[dynedge],bins = bins)
    ax[4,0].set_ylabel('dynedge',size =  20)
    ax[4,0].set_xlabel('retro',size =  20)
    p_x = plot_data[retro].mean()
    p_y = plot_data[dynedge].max() - 2/5*plot_data[dynedge].max()
    ax[4,0].text(p_x,p_y, 'MC' + pid_tag, fontsize=20,  color='red')
    ax[2,0].text(p_x,p_y, '$\\nu$-classified dynedge IC86.11 OscNext', fontsize=15,  color='red')
...
result = plot_data.sort_values('n_strings').reset_index(drop = True)
...
E = result['n_strings'].reset_index(drop = True)
num_bins = 25
fig3 = plt.figure()
...
if plot_osc:
    ...
    ax_osc[2,1].set_xlabel('N Pulses', size = 20)
    #plt.xlabel("$Energy_{log10}$",size = 20)
...
ax[4,1].hist2d(plot_data[retro],plot_data[dynedge],bins = bins)
#ax[2,1].set_ylabel('$predictions_{dynedge}$',size =  20)
ax[4,1].set_xlabel('retro',size =  20)
ax[4,1].text(p_x,p_y, 'MC'  + pid_tag, fontsize=20,  color='red')
...
result = plot_data.sort_values('n_strings')
pred = result[dynedge].reset_index(drop = True)
true = result[variable].reset_index(drop = True)
retro_pred = result[retro].reset_index(drop = True)
E = result['n_strings'].reset_index(drop = True)
...
if variable == 'azimuth':
    ...
    plot_data = pd.concat( [pd.DataFrame(mc_res[dynedge]*const),
                        ...
                        pd.DataFrame(retro_mc['interaction_type']),
                        pd.DataFrame(strings['n_doms'])],axis = 1)
    plot_data.columns = [dynedge,variable,retro,retro_sigma,sigma,'energy_log10','pid','interaction_type', 'n_strings']
...
if(variable != 'energy_log10'):
    ...
    axs_perf[1,0].set_xlabel('N Pulses',size = 30)
    axs_perf[1,1].set_xlabel('N Pulses',size = 30)
