...
def NMAELoss(output,target,w):
    ...
    pred = torch.atan2(output[:,0],output[:,1])
    corr = torch.abs(pred - zenith)/(2*torch.sqrt(k))
...
def Train(model_to_train,graphs_train,device,lr,lr_list,n_workers,batch_size,loss_decimals,graphs_valid,mini_batches_valid):                                                                                
    ...
    loss_func = NMAELoss
    loss_pr_epoch = []
    scaler = pd.read_pickle('/groups/hep/pcs557/speciale/data/raw/dev_level7_mu_e_tau_oscweight_newfeats/meta/transformers.pkl')['truth']['zenith']
        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                ...
                with torch.enable_grad():
                    ...
                    data_train.y[:,5]                    = torch.tensor(scaler.inverse_transform(np.array(data_train.y[:,5].cpu()).reshape(-1,1)),dtype=torch.float).to(device).squeeze(1)
                    model.train()                                                               
                    w                               = data_train.lw
                    optimizer.zero_grad()                                                   
                    out                         = model(data_train)
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            data_pred.y[:,5]                    = torch.tensor(scaler.inverse_transform(np.array(data_pred.y[:,5].cpu()).reshape(-1,1)),dtype=torch.float).to(device).squeeze(1)
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            data.y[:,5]                    = torch.tensor(scaler.inverse_transform(np.array(data.y[:,5].cpu()).reshape(-1,1)),dtype=torch.float).to(device).squeeze(1)
...
if __name__ == '__main__':
    ...
    n_reps          = 1                                                                                                                                    
    lr              = 1e-5#7e-5#7e-4
    max_lr          = 2.5e-4 #2.5e-3#2.5e-4#2.5e-3
    ...
    trained_model,loss_history = Train(Net(args.k).to(device),graphs_train,device,lr,lr_list,n_workers,batch_size,loss_decimals,graphs_valid,mini_batches_valid)
    #trained_model = torch.load('/groups/hep/pcs557/speciale/results/dev_level7_mu_e_tau_oscweight_newfeats/everything_zenithw/dynedge-protov2-zenith-k=8-c3not3-thesis-50epoch-weighted/model.pkl', map_location='cpu').to(device)
