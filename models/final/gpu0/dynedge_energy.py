...
def NMAELoss(output,target,w):
    ...
    u =  target[:,0]
    #sigma = output[:,1]
    loss = torch.sum(torch.log(torch.cosh(((x-u)))))
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            predictions.extend(prediction[:,0].detach().cpu().numpy())
            #sigma.extend(prediction[:,1].detach().cpu().numpy())
            truths.extend(truth)
            count +=1
        ...
        #result.columns  = ['event_no','energy_log10_pred']
        result.columns  = ['event_no','energy_log10','energy_log10_pred']
...
if __name__ == '__main__':
    ...
    base = 'dynedge-protov2-energy-k=%s-c3not3-%s-noprob'%(args.k,args.tag)
    scheduler = 'LinearSchedule'    
    device = torch.device(args.device)     
    batch_size      = 1024#2048 # 824 # 
    n_workers       = 40                                                          
    n_reps          = 1                                                                                                                                    
    lr              = 1e-5#7e-5#7e-4
    max_lr          = 2.5e-4#2.5e-4#2.5e-3
    ...
    trained_model = torch.load('/groups/hep/pcs557/speciale/results/dev_level7_mu_e_tau_oscweight_newfeats/everything_wtest/dynedge-protov2-energy-k=8-c3not3-w_test_val-noprob/model.pkl', map_location='cpu').to(device)
