...
class Net(torch.nn.Module):                                                     
    ...
    def save_graph_picture(self,data,x, edge_index, id):
        save_this = data
        save_this.x = x
        save_this.edge_index = edge_index
        save_path = '/groups/hep/pcs557/speciale/'
        torch.save(save_this,save_path + '%s-graph.pkl'%id)
    ...
    def forward(self, data):
        ...
        #self.save_graph_picture(data,x, edge_index, '0')
        a = self.conv_add(x,edge_index)
        _,edge_index = KNNAmp(k, a, batch)

        #self.save_graph_picture(data,a, edge_index, '1')
        
        b = self.conv_add2(a,edge_index)

        _,edge_index = KNNAmp(k, b, batch)
        
        #self.save_graph_picture(data,b, edge_index, '2')

        c = self.conv_add2(b,edge_index)
        _,edge_index = KNNAmp(k, c, batch)

        #self.save_graph_picture(data,c, edge_index, '3')
        d = self.conv_add2(c,edge_index)
        _,edge_index = KNNAmp(k,d,batch)

        #self.save_graph_picture(data,d, edge_index, '4')
        #self.first = False
...
if __name__ == '__main__':
    ...
    trained_model = torch.load('/groups/hep/pcs557/speciale/results/dev_level7_mu_e_tau_oscweight_newfeats/everything_wtest_testset/dynedge-protov2-zenith-k=8-c3not3-w_test_val/model.pkl', map_location='cpu').to(device)
