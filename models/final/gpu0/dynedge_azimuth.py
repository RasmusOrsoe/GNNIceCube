...
def NMAELoss(output,target,w):
    ...
    u_1 = (1/torch.sqrt(torch.tensor(2,dtype = torch.float)))*torch.sin(azimuth)
    u_2 = (1/torch.sqrt(torch.tensor(2,dtype = torch.float)))*torch.cos(azimuth)
    u_3 = (1/torch.sqrt(torch.tensor(2,dtype = torch.float)))*(1)
    
    norm_x  = torch.sqrt((1)**2 + output[:,0]**2 + output[:,1]**2)
...
def LoadBestParams(model,params):
    ...
    return model        
def Train(model_to_train,graphs_train,device,lr,lr_list,n_workers,batch_size,loss_decimals,graphs_valid,mini_batches_valid,scaler):                                                                                
    count = torch.tensor([0]).to(device)
    model = model_to_train.to(device)
    loss_func = NMAELoss
    loss_pr_epoch = []
    #scaler = pd.read_pickle('/groups/hep/pcs557/databases/dev_level7_mu_e_tau_oscweight_000/data/meta/transformers.pkl')['truth']['azimuth']
        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                ...
                with torch.enable_grad():
                    data_train.y[:,4]                    = torch.tensor(scaler.inverse_transform(np.array(data_train.y[:,4].cpu()).reshape(-1,1)),dtype=torch.float).to(device).squeeze(1)
                    model.train()                                                               
                    w                               = data_train.hw
                    optimizer.zero_grad()                                                   
                    out                         = model(data_train)
            ...
            with torch.no_grad():
                val_loss,q,slaves = Validate(model,graphs_valid,graphs_train,batch_size,o,
                                             q_val,mini_batches_valid,val_slaves,n_workers,mini_batches,manager,epoch,n_epochs,scaler,save = False)
            if epoch == 0:
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
             o,q,mini_batches_valid,val_slaves,n_workers,mini_batches,manager,train_epoch,n_epochs,scaler,save):
    print('in validation')
    model = trained_model
    loss_func = NMAELoss
    target = list() 
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            data_pred.y[:,4]                    = torch.tensor(scaler.inverse_transform(np.array(data_pred.y[:,4].cpu()).reshape(-1,1)),dtype=torch.float).to(device).squeeze(1) #torch.tensor(scaler.inverse_transform(np.array(data_pred.y[:,0]).reshape(-1,1)),dtype=torch.float).to(device)#data_pred.y[:,0].unsqueeze(1)
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
           ...
           # data.y[:,4]                    = torch.tensor(scaler.inverse_transform(np.array(data.y[:,4].cpu()).reshape(-1,1)),dtype=torch.float).to(device).squeeze(1)
        ...
        #                   'azimuth_pred','azimuth_pred_k']
        result.columns  = ['event_no','azimuth_pred','azimuth_pred_k']
...
if __name__ == '__main__':
    ...
    handle = args.graphs #'dev_level7_oscNext_IC86_003/event_only_level7_all_oscNext_IC86_003' #'dev_level7_mu_tau_e_retro_000/event_only_level7_all_neutrinos_retro_SRT_4mio' #
    base = 'dynedge-azimuth-protov2-c3not3-k=%s-%s'%(args.k,args.tag)
    scheduler = 'LinearSchedule'    
    scaler = pd.read_pickle('/groups/hep/pcs557/speciale/data/raw/%s/meta/transformers.pkl'%(args.graphs.split('/')[0]))['truth']['azimuth']
    ...
    n_reps          = 1                                                                                                                                    
    lr              = 1e-5#7e-5#7e-4
    max_lr          = 2.5e-3#2.5e-4#2.5e-3
    end_lr          = 1e-5#7e-5#7e-4                                                               
    n_epochs        = 30 #1990
    patience        = 5
    loss_decimals   = 10
    ...
    trained_model,loss_history = Train(Net(args.k),graphs_train,device,lr,lr_list,n_workers,batch_size,loss_decimals,graphs_valid,mini_batches_valid,scaler)
    #trained_model = torch.load('/groups/hep/pcs557/speciale/results/dev_level7_mu_e_tau_oscweight_newfeats/everything/dynedge-azimuth-protov2-c3not3-k=8-thesis-newlr-30e-check/model.pkl', map_location='cpu').to(device)
    ...
    #    loss,lr = LRFinder(1,Net(args.k),[1e-6,1e-3],NMAELoss,graphs_train,batch_size,mini_batches,n_workers)
