...
class Net(torch.nn.Module):                                                     
    def __init__(self, k):                                                                                   
        ...
        c = 3
        l1, l2, l3, l4, l5,l6,l7 = 8,c*16*2,c*32*2,c*42*2,c*32*2,c*16*2,2
        self.k = k
        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                ...
                with torch.enable_grad():
                    ...
                    loss                            = loss_func(out,data_train.y[:,6].long()) #data_train.mc.long())                            
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            w                               = data_pred.hw 
            loss = loss_func(pred,data_pred.y[:,6].long()) #data_pred.mc.long())#
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            pid.extend(data.y[:,6].unsqueeze(1).detach().cpu().numpy())
            #pid.extend(data.mc.unsqueeze(1).detach().cpu().numpy())
            count +=1
 ...
 #       truths          = pd.DataFrame(np.array(truths).squeeze())
        ...
        result          = pd.concat([pred_events,predictions,pid],axis = 1)
        #result          = pd.concat([pred_events, predictions],axis = 1)
        print(result.shape)
        result.columns  = ['event_no',
                           'out1','out2', 'pid']
        #result.columns  = ['event_no','out1','out2']
...
if __name__ == '__main__':
    ...
    base = 'dynedge-protov2-classification-k=%s-c3not3-%s'%(args.k, args.tag)
    ...
    #trained_model = torch.load('/groups/hep/pcs557/speciale/results/dev_level7_mu_tau_e_muongun_classification/classification_even_sample/dynedge-protov2-classification-k=8-c3not3-classification/model.pkl', map_location='cpu').to(device)
