...
def Train(model_to_train,graphs_train,device,lr,lr_list,n_workers,batch_size,loss_decimals,graphs_valid,mini_batches_valid):                                                                                
    ...
    for p in range(0,n_reps):
        ...
        for epoch in range(0,n_epochs):
            ...
            writer.flush()
            ####
            print('EPOCH: %s / %s || %s / %s min || LR: %s || Loss: %s || Val Loss: %s' %(epoch,n_epochs,(time.time() - start)/60,n_epochs*deltatime,optimizer.param_groups[0]['lr'],round(loss_acc.item()/(mini_batches*batch_size),loss_decimals),round(val_loss.item()/(mini_batches_valid*batch_size),loss_decimals)))
