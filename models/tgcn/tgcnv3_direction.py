...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        self.recurrent_1 = GatedGraphConv(l3,numlayers)
        self.linear_init = torch.nn.Linear(l1, l2)
        self.linear = torch.nn.Linear(l4, l5)
        self.linear1 = torch.nn.Linear(l5, l6)
        self.linear2 = torch.nn.Linear(2*l6,l6)
        self.linear3 = torch.nn.Linear(l6,l7)
        self.linear4 = torch.nn.Linear(l7,l8)
        self.linear_todecode = torch.nn.Linear(l3,l4)
        self.linear_decode = torch.nn.Linear(l4,l4)
        self.linear_toRNN = torch.nn.Linear(l2,l3)
        self.relu = torch.nn.LeakyReLU()
        self.bn1 =torch_geometric.nn.BatchNorm(l4,eps=1e-02)
        self.bn2 =torch_geometric.nn.BatchNorm(l5,eps=1e-02)
    ...
    def forward(self, data):
        ...
        x = self.linear_toRNN(x)
        x = self.recurrent_1(x, edge_index)
        x = self.dropout(x)
        
        x = self.linear_todecode(x)
        
        #x_dec = self.bn1(x)
        x_dec = self.relu(x)
        x_dec = self.linear_decode(x)
        #x_dec = self.bn1(x)
        x_dec = self.relu(x)
        x_dec = self.linear_decode(x)
        x = x + x_dec
...
def Train(model_to_train,graphs_train,device,lr,lr_list,n_workers,batch_size,loss_decimals,graphs_valid,mini_batches_valid):                                                                                
    ...
    for p in range(0,n_reps):
        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                data_train                      = GrabBatch(q)
                data_train.y                    = data_train.y[:,8].unsqueeze(1)#torch.tensor(scaler.inverse_transform(np.array(data_train.y[:,0]).reshape(-1,1)),dtype=torch.float).to(device) #data_train.y[:,0].unsqueeze(1)                                         
                model.train()                                                               
                w                               = 1 
                optimizer.zero_grad()                                                   
                out                         = model(data_train)
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            data_pred.y = data_pred.y[:,8].unsqueeze(1) #torch.tensor(scaler.inverse_transform(np.array(data_pred.y[:,0]).reshape(-1,1)),dtype=torch.float).to(device)#data_pred.y[:,0].unsqueeze(1)

            pred = model(data_pred) 
            loss = loss_func(pred,data_pred.y)
...
def Calculate_Minibatches(graphs_train,graphs_valid,handle,batch_size):
    ...
    check = os.path.isfile(path + '\\' + handle + r'\minibatch\minibatch_%s_GRU.csv'%batch_size)
    if(check == False):
        ...
        pd.DataFrame([mini_batches]).to_csv(path + '\\' + handle + r'\minibatch\minibatch_%s_GRU.csv'%batch_size)
    else:
        print('Mini-batch information for training found at \n %s'%(path + '\\' + handle + r'\minibatch\minibatch_%s_GRU.csv'%batch_size))
        mini_batches = pd.read_csv(path + '\\' + handle + r'\minibatch\minibatch_%s_GRU.csv'%batch_size).values[0][1]
        
    check = os.path.isfile(path + '\\' + handle + r'\minibatch\minibatch_valid_%s_GRU.csv'%batch_size)
    if(check == False):
        ...
        pd.DataFrame([mini_batches_valid]).to_csv(path + '\\' + handle + r'\minibatch\minibatch_valid_%s_GRU.csv'%batch_size,)
    else:
        print('Mini-batch information for validation found at \n %s'%(path + '\\' + handle + r'\minibatch\minibatch_valid_%s_GRU.csv'%batch_size))
        mini_batches_valid = pd.read_csv(path + '\\' + handle + r'\minibatch\minibatch_valid_%s_GRU.csv'%batch_size).values[0][1]
...
if __name__ == '__main__':
    ...
    handle = 'dev_upgrade_train_step4_001\event_only_shuffled_input-_target-_bjørn_weights_wild_gru'
    base = 'tgcnv3-lay5-direction'
   ...
   # pd.DataFrame(loss_history).to_csv(r'J:\speciale\results\runs\results\event_only\%s_loss.csv'%o)
    ...
    ### LR FINDER (n_reps,model,limits,loss_function,graphs_train,batch_size,mini_batches,n_workers)
