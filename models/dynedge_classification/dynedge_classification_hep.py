...
def NMAELoss(output,target,w):
    ...
    loss = loss_func(output, target.squeeze(1).long())
    return loss
start = time.time()                                                            
...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        l1, l2, l3, l4, l5,l6,l7 = 5,64,128,94,64,32,2
        ...
        self.softmax = torch.nn.Softmax(dim = l7)
...
def LRFinder(n_reps,model,limits,loss_function,graphs_train,batch_size,mini_batches,n_workers):
    ...
    for i in range(0,n_reps):
        ...
        for k in range(0,mini_batches):                                               
            ...
            data_train.y                    = data_train.y[:,6]#torch.tensor(scaler.inverse_transform(np.array(data_train.y[:,0]).reshape(-1,1)),dtype=torch.float).to(device) #data_train.y[:,0].unsqueeze(1)                                         
            model_LR.train()                                                               
            w                               = 1 
            optimizer.zero_grad()                                                   
        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                ...
                with torch.enable_grad():
                    ...
                    if(torch.sum(torch.isnan(out)) != 0):
                        print('NAN ENCOUNTERED AT : %s / %s'%(k,mini_batches))                                              
                    loss                            = loss_func(out,data_train.y[:,6].long()) #loss_func(out, data_train.y.float())                             
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            w                               = data_pred.lw 
            loss = loss_func(pred,data_pred.y[:,6].long())#loss_func(pred,data_pred.y)
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            #truth           = data.y[:,6].detach().cpu().numpy()
            pred_events.extend(data.event_no.detach().cpu().numpy())
            predictions.extend(softmax(prediction).detach().cpu().numpy())
            #truths.extend(truth)
            count +=1
...
if __name__ == '__main__':
    ...
    handle = 'dev_level2_real_data_000/event_only_real_data_10mio_no_SRT'
    base = 'dynedge-classification'
    ...
    trained_model = torch.load('/groups/hep/pcs557/speciale/results/dev_classification_corsika_000/event_only_NO_SRT_classification_corsika/dynedge-classification/model.pkl')
