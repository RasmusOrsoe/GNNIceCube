...
def MakeGraph(setting):
    ...
    scalers = pd.read_pickle(r'J:\speciale\data\raw\standard_reco\transformers\transformers.pkl')
...
def GraphGetter(settings):
    holder =list()
    if __name__ == '__main__':
        with Pool(processes = len(settings)) as p:
            data_list = p.map(MakeGraph, settings)
            holder.extend(data_list[:])
            p.close()
            p.join()
    return(holder)
...
if __name__ == '__main__':   
    settings = list()
    events_array = pd.read_csv(r'J:\speciale\data\raw\standard_reco\event_list\events.csv').loc[:,'event_no']
    ...
    #%%
    #graphs_train = GrabGraphs(path = r'J:\speciale\data\graphs\standard\sliced\event_only_shuffle(0,1)(1,4)_retro\train')
    ...
    for p in range(0,n_reps):
        ...
        for epoch in range(n_epochs):
            ...
            medium_batches = np.split(events_array,20)
            for k in range(0,len(medium_batches)):
                holder = list()
                train_list = list()
                event_list = np.array_split(medium_batches[k],4)
                settings = list()
                for k in range(len(event_list)):
                    settings.append(tuple([event_list[k],max_nodes,'J:\\speciale\\data\\graphs\\standard\\sliced','scaled',False,'Time','Standard', False,scaler,False]))
                print('getting holder')
                holder = GraphGetter(settings)
                print('holder gotten')
                for j in range(len(holder)):
                    train_list.extend(holder[j])
                loss_func = NMAELoss                                                          
            ...
            print('EPOCH: %s / %s || %s / %s min || LR: %s || Loss: %s || iter: %s/%s' %(epoch,n_epochs,(time.time() - start)/60,n_epochs*deltatime,optimizer.param_groups[0]['lr'],round(loss_acc.item()/(mini_batches*batch_size),8),count.item(),max_iter))
            if es.step(torch.tensor([round(loss_acc.item()/(mini_batches*batch_size),8)])):
                print('EARLY STOPPING: %s'%epoch)
                break
        
#=============================================================================
    graphs_valid = GrabGraphs(r'J:\speciale\data\graphs\standard\sliced\event_only_shuffle(0,1)(1,4)_retro\valid')
    count = 0
    target = list() 
    res = list()
    model.eval() 
