...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        l1, l2, l3, l4, l5,l6,l7 = 5,5*16,2*32,2*64,2*32,3*16,1
        self.hidden_dim = 3*l3
        self.n_layers = 5
        n_layers = self.n_layers
        
        self.gru = torch.nn.GRU(l3*2,l6, n_layers, dropout=0)
        ...
        self.norm = torch.nn.BatchNorm1d(l6)                                                
        self.nn1 = torch.nn.Linear(l3*2,l4)                                               
        self.nn2   = torch.nn.Linear(l4,l5)
        self.nn3 =  torch.nn.Linear(4*l6,l6)
        self.nn4 = torch.nn.Linear(l6*2,l7)
    ...
    for p in range(0,n_reps):
        ...
        for epoch in range(0,n_epochs):
            ...
            val_loss,q,slaves = Validate(model,graphs_valid,graphs_train,batch_size,o,
                                         q_val,mini_batches_valid,val_slaves,n_workers,mini_batches,manager,h_in,epoch,n_epochs,save = False)
            if epoch == 0:
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
             o,q,mini_batches_valid,val_slaves,n_workers,mini_batches,manager,h_in,train_epoch,n_epochs,save):
    print('in validation')
    model = trained_model
    target = list() 
    res = list()
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            if(dead_check  == True and transition_val == False and train_epoch < n_epochs - 1):
                ...
                transition_val = True
            if( i == mini_batches_valid - 1 and dead_check == False and train_epoch < n_epochs - 1):
...
if __name__ == '__main__':
    ...
    base = 'dynedgeGRU_retro_noweight_baseline_nlay5_withconv_abaggr_nonorm'
