...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        self.nn_conv2 = torch.nn.Sequential(torch.nn.Linear(3*l3*2,l2),
                                            torch.nn.LeakyReLU(),
                                            torch.nn.Linear(l2,l3),
                                            torch.nn.LeakyReLU())
        self.conv_max2 = EdgeConv(self.nn_conv2,aggr = 'max')
        self.conv_mean2 = EdgeConv(self.nn_conv2,aggr = 'mean')
        self.conv_add2 = EdgeConv(self.nn_conv2,aggr = 'add')
    

                                                        
        self.nn1 = torch.nn.Linear(3*3*l3,l4)                                               
        self.nn2   = torch.nn.Linear(l4,l5)
        self.nn3 =  torch.nn.Linear(1*l5,l6)
    ...
    def forward(self, data):                                                    
        ...
        x_1 = self.relu(torch.cat((x_max,x_mean,x_add),dim=1))
        
        
        pos = x_1
        edge_index_2 = self.knn_graph(x=pos,k=self.k,batch=batch)
        x_max = self.conv_max2(x_1,edge_index_2)
        x_mean = self.conv_mean2(x_1,edge_index_2)
        x_add = self.conv_add2(x_1,edge_index_2)

        x_2 = self.relu(torch.cat((x_max,x_mean,x_add),dim=1))
        
        
        pos = x_2
        edge_index_3 = self.knn_graph(x=pos,k=self.k,batch=batch)
        x_max = self.conv_max2(x_2,edge_index_3)
        x_mean = self.conv_mean2(x_2,edge_index_3)
        x_add = self.conv_add2(x_2,edge_index_3)

        x_3 = self.relu(torch.cat((x_max,x_mean,x_add),dim=1))
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    torch.save(model.state_dict(),r'X:\speciale\results\%s\%s\model.pkl'%(handle,baseline))
    predictions     = []
    truths          = []
    pred_events     = []
    manager         = torch.multiprocessing.Manager()
...
if __name__ == '__main__':
    ...
    handle = 'dev_numu_train_l5_retro_001\event_only_aggr_test'
    base = 'dynedgev3-super-coshlog'
    ...
    #trained_model = torch.load(r'X:\speciale\results\dev_upgrade_train_step4_001\event_only_shuffled_input-_target-_less_features_SRT_dom_cutoff\dynedgev3-baseline-4-LeakyReLu-E\model.pkl')
