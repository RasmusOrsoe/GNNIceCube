...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        l1, l2, l3, l4, l5,l6,l7 = 5,16*2,32*2,42*2,32*2,16*2,3
        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                ...
                with torch.enable_grad():
                    ...
                    if(torch.sum(torch.isnan(out)) != 0):
                        print('NAN ENCOUNTERED AT : %s / %s'%(k,mini_batches))                                              
                    loss                            = loss_func(out,data_train.y[:,10].long()) #loss_func(out, data_train.y.float())                             
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            w                               = data_pred.lw 
            loss = loss_func(pred,data_pred.y[:,10].long())#loss_func(pred,data_pred.y)
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            truth           = data.y[:,10:11].detach().cpu().numpy()
        ...
        #result.columns  = ['event_no','E','pid',
        #                   'muon_pred','stopped_muon_pred','v_e_pred','v_u_pred','v_t_pred']
        result.columns  = ['event_no','E','pid',
                           'muon_pred','stopped_muon_pred','v']
...
if __name__ == '__main__':
    ...
    handle = 'dev_classification_000\event_only_classification_muon_stopped_neutrino'
    base = 'dynedgev3-LeakyReLu-classification'
    ...
    trained_model = torch.load(r'X:\speciale\results\dev_classification_000\event_only_classification_muon_stopped_neutrino\dynedgev3-LeakyReLu-classification\model.pkl')
