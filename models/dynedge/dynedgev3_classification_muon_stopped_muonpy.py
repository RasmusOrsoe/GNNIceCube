        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                ...
                with torch.enable_grad():
                    ...
                    if(torch.sum(torch.isnan(out)) != 0):
                        print('NAN ENCOUNTERED AT : %s / %s'%(k,mini_batches))                                              
                    loss                            = loss_func(out,data_train.y[:,11].long()) #loss_func(out, data_train.y.float())                             
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            w                               = data_pred.lw 
            loss = loss_func(pred,data_pred.y[:,11].long())#loss_func(pred,data_pred.y)
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            truth           = data.y[:,10:12].detach().cpu().numpy()
        ...
        result.columns  = ['event_no','E','pid','muon_stopped','muon_pred','muon_stopped_pred']
...
if __name__ == '__main__':
    ...
    handle = 'dev_classification_000\event_only_classification_muon_neutrino'
    base = 'dynedgev3-LeakyReLu-muon_stopped_muon'
    ...
    trained_model = torch.load(r'X:\speciale\results\dev_classification_000\event_only_classification_muon_neutrino\dynedgev3-LeakyReLu-muon_stopped_muon\model.pkl')
