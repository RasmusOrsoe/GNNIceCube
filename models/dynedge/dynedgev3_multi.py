...
def Multi_Loss(output,target):
    mse = MSELoss()
    loss_E = torch.sum(torch.log(torch.cosh((output[:,0] - target[:,0]))))
    loss_azimuth = torch.sum(torch.log(torch.cosh((output[:,1]-target[:,8]))))
    loss_zenith =  torch.sum(torch.log(torch.cosh((output[:,2]-target[:,9]))))
    return loss_E + loss_azimuth + loss_zenith
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        ...
        result.columns  = ['event_no',
                           ...
                           ,'position_y'
                           ,'position_z',
                           'direction_x','direction_y','direction_z','azimuth','zenith','pid','E_pred','azimuth_pred','zenith_pred']
...
if __name__ == '__main__':
    ...
    #trained_model = torch.load(r'X:\speciale\results\dev_numu_train_l5_retro_001\event_only_aggr_test_SRT_energy_scaled_weights\dynedgev3-multi\model.pkl')
