...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        self.clean = TopKPooling(in_channels = l1,min_score = 0.05)
    ...
    def forward(self, data):                                                    
        ...
        x,edge_index = KNNAmp(8, x, batch)

        len1 = len(x)
        x = x, edge_index, _, batch, _, _ = self.clean(x = x,edge_index = edge_index, batch = batch)
        ...
        _,edge_index = KNNAmp(8, a, batch)
        b = self.conv_add2(a,edge_index)

        _,edge_index = KNNAmp(8, b, batch)
        c = self.conv_add2(b,edge_index)

        _,edge_index = KNNAmp(8, c, batch)
...
def LRFinder(n_reps,model,limits,loss_function,graphs_train,batch_size,mini_batches,n_workers):
    ...
    for i in range(0,n_reps):
        ...
        for k in range(0,mini_batches):                                               
            ...
            w                               = 1 
            optimizer.zero_grad()                                                   
            out , ratio , _                     = model(data_train)
            if(torch.sum(torch.isnan(out)) != 0):
                print('NAN ENCOUNTERED AT : %s / %s'%(k,mini_batches))
                                                            
            loss                            = loss_func(out, data_train.y,w,1)                             
            loss.backward()                                                         
            optimizer.step()
...
if __name__ == '__main__':
    ...
    handle = 'dev_upgrade_train_step4_001/event_only_upgrade_no-lc_light_1mio_martin_selection' #'dev_level7_mu_tau_e_retro_000/event_only_level7_all_neutrinos_retro_SRT_4mio' 
    base = 'dynedge-E-protov2-azimuth-semi-sup-+10-no-individual-conv-tightlr'
    ...
    #    loss,lr = LRFinder(1,Net(),[1e-6,1e-4],NMAELoss,graphs_train,batch_size,mini_batches,n_workers)
