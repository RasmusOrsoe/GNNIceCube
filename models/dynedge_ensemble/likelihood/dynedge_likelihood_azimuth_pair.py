...
def NMAELoss(output,target,w):
    ...
    u_3 = (1/torch.sqrt(torch.tensor(2,dtype = torch.float)))*(azimuth/azimuth)
    
    norm_x  = torch.sqrt((azimuth/azimuth)**2 + output[:,0]**2 + output[:,1]**2)
    
    x_1 = (1/norm_x)*output[:,0]
    x_2 = (1/norm_x)*output[:,1]
    x_3 = (1/norm_x)*(azimuth/azimuth)
...
if __name__ == '__main__':
    ...
    base = 'dynedgev3-zenith-likelihood-azimuth-pair'
    ...
    trained_model = torch.load(r'X:\speciale\results\dev_numu_train_l5_retro_001\event_only_aggr_test_SRT_energy_scaled_new_weights\zenith-likelihood-azimuth-pair\model.pkl')
