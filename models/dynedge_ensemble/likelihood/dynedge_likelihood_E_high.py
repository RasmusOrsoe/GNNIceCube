...
def Multi_Loss(output,target,w):
    ...
    loss_E = torch.sum(w*(torch.log(torch.sqrt(2*pi)) + torch.log(torch.abs(sigma) + eps) + 1/2*((x - u)**2/(sigma**2 + eps))))
...
if __name__ == '__main__':
    ...
    base = 'dynedgev3-likelihood-E-hw-100'
    ...
    #trained_model = torch.load(r'X:\speciale\results\dev_numu_train_l5_retro_001\event_only_aggr_test_SRT_energy_scaled_new_weights\dynedgev3-multi-low\model.pkl')
    ...
    ### LR FINDER
    #loss,lr = LRFinder(5,Net(),[1e-8,1e-2],Multi_Loss,graphs_train,batch_size,mini_batches,n_workers)
