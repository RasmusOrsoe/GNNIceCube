...
def NMAELoss(output,target,w):
    ...
    m = 3
    u = target[:,[5,6,7]]
    u = u/torch.sqrt(u[:,0]**2 + u[:,1]**2 + u[:,2]**2).unsqueeze(1)
    x = output[:,[0,1,2]]
    x = x/torch.sqrt(x[:,0]**2 + x[:,1]**2 + x[:,2]**2).unsqueeze(1)
    k = torch.sqrt(x[:,0]**2 + x[:,1]**2 + x[:,2]**2)
    ...
    dotprod = u[:,0]*x[:,0] + u[:,1]*x[:,1] + u[:,2]*x[:,2]
    
    loss = (-torch.log(k)+torch.log(2*pi) + k + torch.log(1-torch.exp(-2*k)))*k - dotprod + l1*k -l2*dotprod
...
if __name__ == '__main__':
    ...
    base = 'dynedgev3-zenith-likelihood-dir-nuh'
    ...
    #trained_model = torch.load(r'X:\speciale\results\dev_numu_train_l5_retro_001\event_only_aggr_test_SRT_energy_scaled_new_weights\dynedgev3-zenith-likelihood-dir\model.pkl')
