...
class BasicBlock(torch.nn.Module):
    def __init__(
        ...
        self.linear = torch.nn.Linear(inplanes,base_width)
        self.linear2 = torch.nn.Linear(base_width,base_width)
        self.linear3  = torch.nn.Linear(base_width,planes)
        #self.conv1 = torch_geometric.nn.GraphConv(inplanes, planes)
        self.bn1 = norm_layer(planes,eps=1e-02,elementwise_affine= True)
        self.relu = torch.nn.LeakyReLU(inplace=True)
        #self.conv2 = torch_geometric.nn.GraphConv(planes, planes)
        self.bn2 = norm_layer(planes,eps=1e-02,elementwise_affine= True)
        self.downsample = torch.nn.Linear(inplanes,planes)
        self.stride = stride
...
def NMAELoss(output,target,w):
    eps = torch.tensor(1e-3)
    azimuth_pred = output[:,0]
    zenith_pred  = output[:,1]
    k            = torch.abs(output[:,2])#torch.tensor(3) #((output[:,2])**2)
    ...
    c1 = torch.cos(zenith_pred - zenith + azimuth_pred - azimuth)
    c2 = torch.cos(zenith_pred + zenith + azimuth_pred - azimuth)
    c3 = torch.cos(zenith_pred - zenith - azimuth_pred + azimuth)
    c4 = torch.cos(zenith_pred + zenith - azimuth_pred + azimuth)
    c5 = torch.cos(zenith_pred - zenith)
    c6 = torch.cos(zenith_pred + zenith)
    co = 0.25*(c1 - c2 + c3 - c4) + 0.5*(c5 + c6)
    logc_3 = -torch.log(k) + k + torch.log(1 - torch.exp(-2*(k)))
    reg2 = -0.1*co
    reg1 = 0.2*(k )
    
    loss = torch.mean(-(k+ eps)*co + logc_3) # + reg2 + reg1)
    
    #print((k.min().item(), k.max().item()), loss.item(),torch.mean(-k*co).item(),torch.mean(logc_3).item()  )
    return loss
...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        self.relu_real = torch.nn.ReLU()
                                          
                                                                                
    def forward(self, data):                                                    
        x, edge_index, batch = data.x, data.edge_index, data.batch
        #x,edge_index = KNNAmp(4, x, batch)
        edge_index = torch.tensor([np.random.randint(0,len(x),len(x)),np.random.randint(0,len(x),len(x))],dtype = torch.long).to(device)           
        ...
        #x,edge_index, _ , batch , _ , _ = self.pool(x,edge_index = edge_index,batch = batch)
...
def LRFinder(n_reps,model,limits,loss_function,graphs_train,batch_size,mini_batches,n_workers):
    ...
    for i in range(0,n_reps):
        ...
        for k in range(0,mini_batches):                                               
            ...
            #data_train.y                    = data_train.y[:,5:10]#torch.tensor(scaler.inverse_transform(np.array(data_train.y[:,0]).reshape(-1,1)),dtype=torch.float).to(device) #data_train.y[:,0].unsqueeze(1)                                         
            model_LR.train()                                                               
            w                               = 1 
            optimizer.zero_grad()                                                   
        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                ...
                with torch.enable_grad():
                    ...
                    loss                            = loss_func(out,data_train.y,w) #loss_func(out, data_train.y.float())                             
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            #data_pred.y = data_pred.y[:,5:10] #torch.tensor(scaler.inverse_transform(np.array(data_pred.y[:,0]).reshape(-1,1)),dtype=torch.float).to(device)#data_pred.y[:,0].unsqueeze(1)
            pred = model(data_pred)
            w                               = data_pred.lw 
            loss = loss_func(pred,data_pred.y,w)#loss_func(pred,data_pred.y)
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            truth           = data.y[:,5:10].detach().cpu().numpy()
        ...
        result.columns  = ['event_no','E','dir_x','dir_y','dir_z','azimuth','zenith',
                           'azimuth_pred','zenith_pred','k']
        result.to_csv(path + '\\' + 'results_%s.csv'%o)
        
        #os.makedirs(r'J:\speciale\results\%s\%s'%(handle,baseline),exist_ok = True)
        ...
        print('Result Saved at: \n \
              reports: %s \
              archive: %s '%(path + '\\' + 'results_%s.csv'%o, r'J:\speciale\results\%s\%s'%(handle,baseline)))
...
if __name__ == '__main__':
    handle = 'dev_numu_train_l5_retro_001\event_only_aggr_test_SRT_energy_scaled_new_weights'
    base = 'likelihood-az-zen-no-eps-noreg-rando'
    ...
    trained_model = torch.load(r'X:\speciale\results\dev_numu_train_l5_retro_001\event_only_aggr_test_SRT_energy_scaled_new_weights\likelihood-az-zen-no-eps-noreg-rando\model.pkl')
    ...
    #    loss,lr = LRFinder(1,Net(),[1e-7,1e-2],NMAELoss,graphs_train,batch_size,mini_batches,n_workers)
    #    loss_list.append(loss)
    #    lr_list.append(lr)
    #for l in range(0,len(loss_list)):
    #    fig = plt.figure()
    #    plt.plot(np.array(lr_list[l]),np.array(np.log(loss_list[l])).squeeze())
