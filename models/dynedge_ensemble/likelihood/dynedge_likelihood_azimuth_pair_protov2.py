...
def NMAELoss(output,target,w):
    ...
    azimuth = target[:,4]
    #zenith  = target[:,9]
    #scaler = torch.load('/groups/hep/pcs557/databases/dev_level7_mu_e_tau_oscweight_000/data/meta/transformers.pkl')['truth']['azimuth']
    #azimuth = torch.tensor(scaler.inverse_transform(np.array(azimuth.cpu()).reshape(-1,1)),dtype=torch.float).to(device)
...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        #self.nn_clean1 = torch.nn.Linear(l1,1)
        #self.nn_clean2 = torch.nn.Linear(124*250,124*250)
        ...
        self.relu_real = torch.nn.LeakyReLU()
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            truth           = data.y[:,1:6].unsqueeze(1).detach().cpu().numpy()
            pred_events.extend(data.event_no.unsqueeze(1).detach().cpu().numpy())
        ...
        #print(data.y[:,5:].unsqueeze(1).detach().cpu().numpy())
        ...
        #result.columns  = ['event_no','azimuth_pred','azimuth_k']
...
if __name__ == '__main__':
    ...
    handle = 'dev_level7_mu_e_tau_oscweight_000/event_only_level7_all_oscweight_muon'
    base = 'dynedge-E-protov2-azimuth-noscale-muon'
    ...
    trained_model = torch.load('/groups/hep/pcs557/speciale/results/dev_level7_mu_e_tau_oscweight_000/event_only_level7_all_oscweight_muon/dynedge-E-protov2-azimuth-noscale-muon/model.pkl')
