...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        self.pool = EdgePooling(in_channels = l3,dropout= 0, add_to_edge_score = 0, edge_score_method = EdgePooling.compute_edge_score_tanh)
        
        self.pool2 = EdgePooling(in_channels = l3, dropout= 0, add_to_edge_score = 0, edge_score_method = EdgePooling.compute_edge_score_tanh)
    ...
    def forward(self, data):                                                    
        ...
        x,edge_index = KNNAmp(4, x, batch)
        x = self.conv_add2(x,edge_index)
        x,edge_index,batch, info = self.pool(x,edge_index,batch)
        x,edge_index = KNNAmp(4, x, batch)
        x = self.conv_add2(x,edge_index)
        #x,edge_index,batch, info = self.pool2(x,edge_index,batch)
        x,edge_index = KNNAmp(4, x, batch)
        x = self.conv_add2(x,edge_index)
...
if __name__ == '__main__':
    ...
    base = 'likelihood-az-zen-no-eps-noreg-proto-pool'
