...
def ModBessel(k,m):
    ...
    for i in range(0,m+1):
        fact = torch.tensor(np.math.factorial(i)).to(device)
        res += (k**(2*i))/(2**(2*i)*(fact)**2)
    return res.to(device)
def NMAELoss(output,target):
    ...
    loss = -k*torch.cos(x-u) + torch.log(2*pi) + torch.log(1 + k**2/4 + (k**4)/((2**4)*4)) 
...
class Net(torch.nn.Module):                                                     
    def __init__(self):                                                                                   
        ...
        l1, l2, l3, l4, l5,l6,l7 = 5,3*16*2,3*32*2,3*42*2,3*32*2,3*16*2,2
...
def LRFinder(n_reps,model,limits,loss_function,graphs_train,batch_size,mini_batches,n_workers):
    ...
    for i in range(0,n_reps):
        ...
        for k in range(0,mini_batches):                                               
            ...
            loss                            = loss_func(out, data_train.y.unsqueeze(1))                             
            loss.backward()                                                         
            optimizer.step()
        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                ...
                with torch.enable_grad():
                    ...
                    loss                            = loss_func(out,data_train.y[:,9].unsqueeze(1)) #loss_func(out, data_train.y.float())                             
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            w                               = data_pred.lw 
            loss = loss_func(pred,data_pred.y[:,9].unsqueeze(1))#loss_func(pred,data_pred.y)
...
if __name__ == '__main__':
    ...
    base = 'dynedgev3-zenith-likelihood'
    ...
    #trained_model = torch.load(r'X:\speciale\results\dev_upgrade_train_step4_001\event_only_shuffled_input-_target-_cleaned\dynedgev3-baseline-4-LeakyReLu-zenith_upgrade\model.pkl')
    ...
    ### LR FINDER
    #loss,lr = LRFinder(5,Net(),[1e-7,1e-1],DirectionLoss,graphs_train,batch_size,mini_batches,n_workers)
