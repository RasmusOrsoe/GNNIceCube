...
def Multi_Loss(output,target,w):
    mse = MSELoss()
    #loss_E = torch.sum(w*torch.log(torch.cosh((output[:,0] - target[:,0]))))
    #loss_azimuth = torch.sum(w*torch.log(torch.cosh((output[:,1]-target[:,8]))))
    #loss_zenith =  torch.sum(w*torch.log(torch.cosh((output[:,2]-target[:,9]))))
    ...
    #loss_E =  (1/len(x))*torch.sum(((x-u)**2)/(sigma**2 + eps)+torch.log(sigma**2 + eps))
    
    gauss_exp = torch.log(torch.sqrt(2*pi)) + torch.log(torch.abs(sigma) + eps) + 1/2*((x - u)**2/(sigma**2 + eps))
    loss_E = torch.mean(gauss_exp)
    
    #loss_E = -torch.log(torch.prod(gauss))
    
    #loss_E  = torch.sum((1/sigma**2)*(x-u)**2 + 1/2*torch.log(2*pi*sigma**2))
    #loss_E = torch.log(output[:,0]) + 1/2*((output[:,1] - target[:,0])/output[:,0])**2
    return loss_E
...
def LRFinder(n_reps,model,limits,loss_function,graphs_train,batch_size,mini_batches,n_workers):
    ...
    optimizer_init = torch.optim.Adam(model.parameters(), lr=lr_list[0],eps = 1e-3)
    result = []
        ...
        for epoch in range(0,n_epochs):
            ...
            for k in range(0,mini_batches):                                               
                ...
                with torch.enable_grad():
                    #data_train.y                    = data_train.y[:,[0,9]]#torch.tensor(scaler.inverse_transform(np.array(data_train.y[:,0]).reshape(-1,1)),dtype=torch.float).to(device) #data_train.y[:,0].unsqueeze(1)                                         
                    model.train()                                                               
                    w                               = data_train.lw
                    optimizer.zero_grad()                                                   
                    out                         = model(data_train)
...
def Validate(trained_model,graphs_valid,graphs_train,batch_size,
    ...
    with torch.no_grad():
        for i in range(0, mini_batches_valid):                                                  
            ...
            #data_pred.y = data_pred.y[:,[0,9]] #torch.tensor(scaler.inverse_transform(np.array(data_pred.y[:,0]).reshape(-1,1)),dtype=torch.float).to(device)#data_pred.y[:,0].unsqueeze(1)
...
def Predict(model,prediction_graphs,n_workers,pred_mini_batches,batch_size,o,handle,baseline):
    ...
    with torch.no_grad():
        for mini_batch in range(0,pred_mini_batches):
            ...
            truth           = data.y[:,0:11].detach().cpu().numpy()
        ...
        result.columns  = ['event_no',
                           'E',
                           'time',
                           'position_x'
                           ,'position_y'
                           ,'position_z',
                           'direction_x','direction_y','direction_z','azimuth','zenith','pid','E_pred','E_sig']
...
if __name__ == '__main__':
    ...
    base = 'dynedgev3-likelihood-E-nw-const_lr'
    ...
    #trained_model = torch.load(r'X:\speciale\results\dev_numu_train_l5_retro_001\event_only_aggr_test_SRT_energy_scaled_new_weights\dynedgev3-likelihood-E-nw-const_lr\model.pkl')
    ...
    for n in range(0,n_reps):    
        loss,lr = LRFinder(1,Net(),[1e-7,1e-2],Multi_Loss,graphs_train,batch_size,mini_batches,n_workers)
